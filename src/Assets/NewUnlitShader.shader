        ...
        Pass
        {
            ...
            float4 frag(v2f_customrendertexture i) : SV_Target
            {
                ...
                {// 自分が生きている
                    ...
                    if(3 == num_alive || num_alive == 4) ret = tex2D(_SelfTexture2D, uv).rgb;
                    // 過疎：隣接する生きたセルが1つ以下ならば、過疎により死滅する。
                    if(1 >= num_alive) ret = float3(0,0,0);
                    // 過密：隣接する生きたセルが4つ以上ならば、過密により死滅する。
                    if(6 <= num_alive) ret = float3(0,0,0);// 黒
                    if(5 == num_alive) ret = float3(1,1,1);
                    if(2 == num_alive) ret = float3(1,0,1);
